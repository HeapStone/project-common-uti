...
public static Object setBeanProperty (Map<String,Object> values,Class<?> clazz ){
			...
			for(String key :keys){
				...
				if(null != method ){
					...
					if(null!=argsClass && argsClass.length>0){
						...
						throw new RuntimeException(clazz.getName()+ "属性["+key+"] setter方法参数类型不匹配！");
					}
					
				}else{
					throw new RuntimeException(clazz.getName()+ "属性["+key+"] 未定义setter方法!");
				}
			}
		}
	...
	public static Object setBeanProperty (Map<String,Object> values,Object resobj){
		if(null != values){
			Set<String> keys = values.keySet();
			for(String key :keys){
				PropertyDescriptor pd = null;
				try {
					pd = new PropertyDescriptor(key,resobj.getClass());
					//得到get方法
					Method method = pd.getWriteMethod();
					if(null != method ){
						Class[] argsClass = method.getParameterTypes();
						if(null!=argsClass && argsClass.length>0){
							//类型转换
							//调用set方法赋值
							method.invoke(resobj, new Object[]{convertProperType(argsClass[0].getName(),values.get(key))});
						}else{
							throw new RuntimeException(resobj.getClass().getName()+ "属性["+key+"] setter方法参数类型不匹配！");
						}

					}else{
						throw new RuntimeException(resobj.getClass().getName()+ "属性["+key+"] 未定义setter方法!");
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
...
private static Object convertProperType(String methodType,Object ob){
	...
	if( "java.math.BigDecimal".equalsIgnoreCase(methodType)){
        ...
        try {
            return DateUtil.getDate(ob.toString());
        } catch (Exception e) {
            e.printStackTrace();
        }
	 ...
	 * @return
	 */
	public static Object getBeanProperties(Object obj,String fileName){
	Object resObj =null;
	Map<String,Object> remaps = getBeanProperty(obj);
	if(null!=remaps&&!remaps.isEmpty()){
		resObj=remaps.get(fileName);
	}
	return resObj;
}
}
